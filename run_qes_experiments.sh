...
if [[ -z "${BACKBONE_FILE}" ]]; then
  BACKBONE_FILE="$(pick_first_existing \
    /root/autodl-tmp/QES6D/QES6D/RepVGG-B1g2-train.pth \
    ${PROJECT_DIR}/weights/RepVGG-B1g2-train.pth \
    ${PROJECT_DIR}/QES6D/RepVGG-B1g2-train.pth || true)"
fi
...
if [[ -z "${YOLO_WEIGHTS}" ]]; then
  YOLO_WEIGHTS="$(pick_first_existing \
    ...
    ${PROJECT_DIR}/facedat/yolov8x-face.pt \
    ${PROJECT_DIR}/facedat/yolov8n-face.pt \
    /root/autodl-tmp/QES6D/facedat/yolov8x.pt \
    ${PROJECT_DIR}/facedat/yolo11x.pt || true)"
fi
...
CONSENSUS_AUX_MODEL_VARIANT="${CONSENSUS_AUX_MODEL_VARIANT:-QES6D_o}"
...
FAIR_ONLY_CUSTOM_QES6D="${FAIR_ONLY_CUSTOM_QES6D:-0}"
...
if [[ "${RUN_MODE}" != "train_only" ]]; then
  ...
  GPU_ID="${GPU_ID}" \
  SEED="${SEED}" \
  CUSTOM_QES6D_ENABLE=1 \
  CUSTOM_QES6D_CKPT="${MODEL_FOR_EVAL}" \
  CUSTOM_QES6D_MODEL_VARIANT="${MODEL_VARIANT}" \
  CUSTOM_QES6D_YOLO_WEIGHTS="${YOLO_WEIGHTS}" \
  CUSTOM_QES6D_GFPGAN_MODEL="${GFPGAN_MODEL}" \
  ONLY_CUSTOM_QES6D="${FAIR_ONLY_CUSTOM_QES6D}" \
